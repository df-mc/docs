...
```go title="invalid_transaction.go"
var w *world.World
w.Exec(func(tx *world.Tx) {
    // tx is valid here.
    go func() {
...
```go title="entity_transaction.go"
var w *world.World
...
```go title="old_accept.go"
for srv.Accept(func(p *player.Player) {
	// Use p.
})
```
to:
```go title="new_accept.go"
